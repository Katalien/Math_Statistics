    ...
    for dist in ds.distributions:
        for size in samle_sizes:
            for i in range(repeats):
                ...
                E_mediana = np.around(np.mean(median), decimals=4)
                ...
                D_mediana = np.around(np.mean(np.multiply(median, median)) - np.mean(median) * np.mean(median), decimals=4)
            ...
            with open("lab2_res/" + dist + str(size) + ".tex", "w") as f:
                ...
                mean_left, mean_right = np.around(E_mean - sqrt(D_mean), decimals=1), np.around(E_mean + sqrt(D_mean), decimals=1)
                mediana_left, mediana_right = np.around(E_mediana - sqrt(D_mediana), decimals=1), np.around(E_mediana + sqrt(D_mediana), decimals = 1)
                z_r_left, z_r_right = np.around(E_z_r - sqrt(D_z_r), decimals=1), np.around(E_z_r + sqrt(D_z_r), decimals = 1)
                z_q_left, z_q_right = np.around(E_z_Q - sqrt(D_z_Q), decimals=1), np.around(E_z_Q + sqrt(D_z_Q), decimals=1)
                z_tr_left, z_tr_right = np.around(E_z_tr - sqrt(D_z_tr), decimals=1), np.around(E_z_tr + sqrt(D_z_tr), decimals=1)
